...
def get_start_time(line_iterable, year):
    ...
    for line in line_iterable:
        line = line.strip()
        if line.find('Iteration 0, loss =') != -1 or line.find('Iteration 0 (') != -1: #line.find('Solving') != -1:
...
def extract_seconds(input_file, output_file):
    ...
    regex_iteration = re.compile('Iteration \d+, loss =')
    regex_iteration2 = re.compile('Iteration \d+ \(.*\), loss =')
    out = open(output_file, 'w')
    for line in lines:
        line = line.strip()
        if regex_iteration.search(line) or regex_iteration2.search(line):#line.find('Iteration') != -1:
